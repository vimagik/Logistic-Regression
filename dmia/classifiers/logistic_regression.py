...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            order_number = np.random.choice(num_train, size=batch_size)
            X_batch = X[order_number]
            y_batch = y[order_number]
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_proba = np.vstack((1 - probability, probability)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        probability = 1 / (1 + np.exp(-X_batch.dot(self.w)))
        loss = -y_batch.dot(np.log(probability)) - (1 - y_batch).dot(np.log(1 - probability))
        dw = (probability - y_batch) * X_batch
